...
class CreateHtmlFromProgramming:
    ...
    def _generate_triplicate_and_subtitle(self, html, dictionary):
        ...
        checked = 'checked' if dictionary[config.WORKING_IN_TRIPLICATE] else ''
        ...
        for i in range(1, dictionary[config.VALVES_QUANTITY] + 1):
            html += f'<th id="{i}" scope="col" style="font-size:17px">{i} <p onclick="ChangeToInput(this)" '
            name = f'Valv{i}' if dictionary[config.SUBTITLE].get(
                f'Valv{i}') is None else dictionary[config.SUBTITLE].get(f'Valv{i}')
    ...
    def _generate_table(self, html, dictionary):
        html += '<tbody>'
        for i in range(0, len(dictionary[config.PROGRAMMING])):
            html += '' if i == 0 else self._generate_white_line(dictionary)
            grupo = dictionary[config.PROGRAMMING][f'{config.GROUP_NUMBER}{i}']
            for j in range(0, len(grupo)):
                linha = grupo[f'{config.LINE_NUMBER}{j}']
                html += '<tr align="center" class="valve_data"> '
                checked = 'checked' if linha[config.CYCLE_START] else ''
                html += f'<td class="align-middle"> <input type="checkbox" class="cycle_checkbox" {checked}> </td> '
                html += '<td class="align-middle"> '
                value = linha[config.SLEEP_TIME] or 0
                ...
                for k in range(0, dictionary[config.VALVES_QUANTITY]):
                    checked = ''
                    if (k + 1) in linha[config.OPEN_VALVES]:
                        checked = 'checked'
    ...
    def _generate_white_line(self, dictionary):
        ...
        for _ in range(0, dictionary[config.VALVES_QUANTITY] + 3):
