...
class Webcam():
  ...
  def __init__(self):
    self.current_port = 0 # Variável global que armazena a porta atualmente usada pela webcam (inicia como porta 0).
    self.rectangle = Rectangle() # Variável global que armazena a porta atualmente usada pela webcam (inicia como porta 0).
    self.uploaded_frame = Frame() # Variável global que armazena o frame enviado pelo usuário (opcional - para o diferenciador).
    self.captured_frame = Frame() # Variável global que armazena o último frame capturado pela webcam.
    self.video_capture = VideoCapture(self.captured_frame.set_frame) # Encapsula a manipulação direta da instância OpenCV do VideoCapture.
